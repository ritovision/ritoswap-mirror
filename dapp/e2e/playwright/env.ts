...
//
// Loads .env.playwright, validates with Zod, derives the wallet address
...
type NetworkInput = 'Ethereum' | 'Sepolia' | 'Ritonet';
type Network = 'ethereum' | 'sepolia' | 'ritonet';

// --- ESM-safe __dirname / __filename ---
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// ---- Load .env.playwright from dapp root ----
// current file: dapp/e2e/playwright/env.ts
// two levels up from here = dapp/.env.playwright
...
if (!loadedPath) {
  console.warn('[e2e env] .env.playwright not found. Using process.env as-is.');
}

// ---- Zod Schemas ----
const NetworkEnum = z.enum(['Ethereum', 'Sepolia', 'Ritonet']).transform((v) => v as NetworkInput);
...
const derivedChainId = normalizedNetwork !== 'ritonet' ? defaultChainIds[normalizedNetwork] : undefined;
const chainId = raw.CHAIN_ID ?? derivedChainId;

if (normalizedNetwork === 'ritonet' && (chainId === undefined || chainId === null)) {
  throw new Error('[e2e env] CHAIN_ID is required when NETWORK=Ritonet (custom network)');
}
...
export const walletConfigFromEnv = {
  ...
  walletIcon: '/images/SEO/favicon.png',
